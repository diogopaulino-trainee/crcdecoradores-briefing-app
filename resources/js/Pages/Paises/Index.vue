...
const novo = () => router.get(route('paises.create'));
const editar = (id) => router.get(route('paises.edit', id));
const confirmarEliminar = (id) => {
    const pais = props.paises.data.find((p) => p.id === id);
    if (pais) {
        paisAEliminar.value = pais;
        showModal.value = true;
    }
};

const eliminar = () => {
    if (paisAEliminar.value) {
        console.log('A eliminar:', paisAEliminar.value);
        router.delete(route('paises.destroy', paisAEliminar.value.id), {
            onSuccess: () => router.get(route('paises.index')),
        });
    }
};
...
const columns = [
    ...
    },
    {
        accessorKey: 'codigo',
        header: () =>
            h('div', { class: 'flex items-center gap-1 cursor-pointer', onClick: () => sortBy('codigo') }, [
                'Código',
                currentSort.value === 'codigo' ? h(currentDirection.value === 'asc' ? ArrowUp : ArrowDown, { class: 'w-4 h-4' }) : null,
            ]),
        cell: ({ row }) => h('span', row.codigo || '—'),
    },
    {
        id: 'acoes',
        header: 'Ações',
...
<template>
    <Head title="Países - CRCDecoradores" />
    ...
    <AppLayout>
        <div class="mb-4 flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between">
                <div class="text-sm text-gray-600">{{ paises.total }} resultado(s)</div>
                <div class="flex gap-2">
                    <Button variant="outline" @click="limparFiltros">Limpar</Button>
                    <Button @click="novo">Novo País</Button>
                </div>
            </div>
        </div>

        <DataTable :columns="columns" :data="paises.data" class="rounded border shadow" />

        <!-- Paginação -->
        <div class="mt-4 flex justify-center">
            <ul class="flex gap-1">
                <li v-for="link in paises.links" :key="link.label">
                    <button
                        v-if="link.url"
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div class="rounded-lg bg-white p-6 shadow-xl">
                ...
                <p class="mb-6">
                    Tens a certeza que queres eliminar o país
                    <strong class="text-[#CDAA62]">{{ paisAEliminar?.nome }}</strong
                    >?
                </p>
