...
const submit = () => {
    ...
    form.post(route('permissoes.store'), {
        onSuccess: () => {
            toast.success('Grupo criado com sucesso!');
            router.visit(route('permissoes.index'));
        },
        onError: () => toast.error('Erro ao criar o grupo.'),
    });
};
</script>
...
<template>
    ...
    <AppLayout>
        <div class="mx-auto max-w-3xl space-y-6 rounded bg-white p-8 shadow">
            <h2 class="text-2xl font-bold text-[#CDAA62]">Criar Grupo de Permissões</h2>
