    ...
    <div class="flex flex-col gap-6">
        ...
        <RadioGroup default-value="starter" class="max-w-sm">
            <Label
                v-for="plan in plans"
                :key="plan.id"
                class="flex items-start gap-3 rounded-lg border p-4 hover:bg-accent/50 has-[[data-state=checked]]:border-green-600 has-[[data-state=checked]]:bg-green-50 dark:has-[[data-state=checked]]:border-green-900 dark:has-[[data-state=checked]]:bg-green-950"
            >
                <RadioGroupItem
                    :id="plan.name"
                    :value="plan.id"
