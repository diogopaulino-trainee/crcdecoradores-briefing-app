...
import { FormControl, FormDescription, FormField, FormItem, FormLabel, FormMessage } from '@/registry/new-york-v4/ui/form';
...
<template>
    <form class="grid w-full max-w-sm gap-6" @submit="onSubmit">
        ...
        <FormField v-slot="{ componentField }" name="mobile">
            <FormItem class="shadow-xs flex flex-row items-start gap-3 rounded-md border p-4">
                <FormControl>
                    <Checkbox :model-value="componentField.modelValue" @update:model-value="componentField['onUpdate:modelValue']" />
                </FormControl>
                <div class="flex flex-col gap-1">
                    <FormLabel class="leading-snug"> Use different settings for my mobile devices </FormLabel>
                    <FormDescription class="leading-snug"> You can manage your mobile notifications in the mobile settings page. </FormDescription>
                </div>
            </FormItem>
        </FormField>
        <FormField v-slot="{ field }" name="items">
            <FormItem class="flex flex-col gap-4">
                ...
                <div class="flex flex-col gap-2">
                    <FormField v-for="item in items" :key="item.id" name="items">
                        <FormItem :key="item.id" class="flex items-start gap-3">
                            <FormControl>
                                <Checkbox
                            ...
                            <FormLabel class="text-sm font-normal leading-tight">
                                {{ item.label }}
                            </FormLabel>
                        </FormItem>
                    </FormField>
            ...
            <FormItem class="flex flex-col">
                ...
                <Popover>
                    <PopoverTrigger as-child>
                        <FormControl>
                            <Button variant="outline" :class="cn('w-[240px] pl-3 text-left font-normal', !value && 'text-muted-foreground')">
                                <span>
        ...
        <div>
            ...
            <div class="flex flex-col gap-4">
                <FormField v-slot="{ componentField }" name="marketing_emails">
                    <FormItem class="shadow-xs flex flex-row items-start justify-between rounded-lg border p-4">
                        <div class="flex flex-col gap-0.5">
                            <FormLabel class="leading-normal"> Marketing emails </FormLabel>
                            <FormDescription class="leading-snug"> Receive emails about new products, features, and more. </FormDescription>
                        </div>
                        <FormControl>
                            <Switch :model-value="componentField.modelValue" @update:model-value="componentField['onUpdate:modelValue']" />
                        </FormControl>
                    </FormItem>
                </FormField>
                <FormField v-slot="{ componentField }" name="security_emails">
                    <FormItem class="shadow-xs flex flex-row items-start justify-between rounded-lg border p-4">
                        <div class="flex flex-col gap-0.5 opacity-60">
                            <FormLabel class="leading-normal"> Security emails </FormLabel>
                            <FormDescription class="leading-snug"> Receive emails about your account security. </FormDescription>
                        </div>
                        <FormControl>
                            <Switch
                                :model-value="componentField.modelValue"
